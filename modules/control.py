...
def kill_php_proc():
    with open("nocontrol-web/Settings.json", "r") as jsonFile:
    ...
    try:
        for i in pid:
            ...
            pid.clear()
            data["pid"] = []
            with open("nocontrol-web/Settings.json", "w") as jsonFile:
...
def run_php_server(port):
    with open(f"nocontrol-web/log/php-{md5_hash().hexdigest()}.log","w") as php_log:
        proc_info = subprocess.Popen(("php","-S",f"localhost:{port}","-t","nocontrol-web"),stderr=php_log,stdout=php_log).pid
